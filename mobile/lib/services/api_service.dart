...
class ApiService {
  ...
  static const String baseUrl = 'http://10.86.5.93:8000';
  ...
  static Future<Map<String, dynamic>> uploadEncryptedBill({
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      throw Exception(
          'Failed to upload bill: ${response.statusCode} - ${response.body}');
    }
  }
  ...
  static Future<List<dynamic>> getWarranties() async {
    final response =
        await http.get(Uri.parse('$baseUrl/warranties/'), headers: {
      'X-User-ID': userId,
    });
